...
config = {
    ...
    "SCREEN_WIDTH_MM": 155,         # Width of the screen's active area in mm (e.g., 344 for a typical monitor)
    "SCREEN_HEIGHT_MM": 87,        # Height of the screen's active area in mm (e.g., 194 for a typical monitor)

    # LED pitch (spacing between centers of LEDs)
    "LED_PITCH_MM": 5,              # Desired pitch in mm (5mm for your case)
...
cell_spacing_x = int(pixels_per_mm_x * config["LED_PITCH_MM"] + 0.5)  # Rounded spacing in pixels (x)
cell_spacing_y = int(pixels_per_mm_y * config["LED_PITCH_MM"] + 0.5)  # Rounded spacing in pixels (y)

# Visible grid size (crop to what fits on screen at true scale)
num_cols_visible = min(config["COLS"], screen_w // cell_spacing_x)
num_rows_visible = min(config["ROWS"], screen_h // cell_spacing_y)
...
first_center_x = (screen_w - (num_cols_visible - 1) * cell_spacing_x) // 2
first_center_y = (screen_h - (num_rows_visible - 1) * cell_spacing_y) // 2

# Radius of each dot (based on spacing, minimum 1)
radius = min(cell_spacing_x // 2 - 1, cell_spacing_y // 2 - 1)
if radius < 1:
    radius = 1
            ...
            for vis_x in range(num_cols_visible):
                ...
                if v > 0:
                    color = (int(base_r * v),
                             int(base_g * v),
                             int(base_b * v))
                    px = first_center_x + vis_x * cell_spacing_x
                    py = first_center_y + vis_y * cell_spacing_y
